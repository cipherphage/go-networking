...
import (
	...
	"github.com/cipherphage/go-networking/ch09/handlers"
	"github.com/cipherphage/go-networking/ch09/middleware"
)

var (
	addr = flag.String("listen", "127.0.0.1:8080", "listen address")
	cert = flag.String("testcert", "", "certificate")
	pkey = flag.String("testkey", "", "private key")
					...
					if pusher, ok := w.(http.Pusher); ok {
						...
						for _, target := range targets {
							if err := pusher.Push(target, nil); err != nil {
								log.Println("%s push failted: %v", target, err)
							}
						}
					}
