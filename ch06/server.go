...
///////////////////////
// Server notes:
// The fact that our packet types implement the encoding.BinaryMarshaler
// and encoding.BinaryUnmarshaler interfaces means that our server code
// can act as a conduit between the network interface and these types,
// leading to simpler code. All our server must concern itself with is
// transfering byte slices between our types and the network connection.
// The logic in the type interfaces takes care of the rest.
///////////////////////

type Server struct {
	Payload []byte        // The payload served for all read requests.
	Retries uint8         // The number of times to retry sending a packet.
	Timeout time.Duration // The amount of time to wait for an ack.
}
...
NEXTPACKET:
	...
	RETRY:
		for i := s.Retries; i > 0; i-- {
			...
			// Wait fot the client's ACK packet.
